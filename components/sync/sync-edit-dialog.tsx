    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-surface rounded-2xl border border-border shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-shrink-0">
          <div>
            <div className="text-sm font-bold text-ink">Edit sync setup</div>
            <div className="text-xs text-slate mt-0.5">Changes take effect on the next run</div>
          </div>
          <button type="button" onClick={onClose} className="text-slate hover:text-ink p-1 rounded transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 flex flex-col gap-5">
          ...
          <div>
            ...
            <input
              ...
              className="w-full text-sm bg-background border border-border rounded-lg px-3 py-2 outline-none focus:border-electric focus:ring-2 focus:ring-electric/10"
            />
          </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2">
              {sources.map((src) => (
                ...
                >
                  <input type="checkbox" readOnly checked={selectedSources.includes(src.id)} className="accent-electric w-4 h-4 flex-shrink-0" />
                  <span className={cn("text-sm font-medium", selectedSources.includes(src.id) ? "text-deep" : "text-ink")}>
                    {src.name}
                  </span>
                </button>
              ))}
          ...
          {/* Filter rules */}
          <div>
            <label className="text-xs font-semibold text-ink block mb-2">Filter rules</label>
            <div className="flex flex-col gap-2 mb-2">
              {filterRules.map((rule, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  />
                  <button type="button" onClick={() => removeFilter(i)} className="text-slate hover:text-red-600 p-1 rounded transition-colors">
        ...
        <div className="px-6 py-4 border-t border-border flex items-center justify-between flex-shrink-0">
          <button type="button" onClick={onClose} className="text-sm text-slate hover:text-ink transition-colors">
            Cancel
          </button>
          <button
            type="button"
            ...
            className={cn(
              ...
              canSave ? "bg-electric text-white hover:bg-electric/90" : "bg-border text-slate cursor-not-allowed"
            )}
          >
