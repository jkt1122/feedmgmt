...
export function PipelinePanel({
  ...
  return (
    <div className="border-b border-border bg-card flex-shrink-0">
      ...
      {panelOpen && (
        <div className="border-t border-border/50 bg-accent/40">
          ...
          {globalRules.length > 0 && (
            <RuleSection title="Your global rules" count={globalRules.length} icon={<Globe className="w-3 h-3 text-primary" />}>
            ...
            {DEFAULT_RULES.map((rule) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={cn("text-sm truncate flex-1", isDisabled ? "line-through text-muted-foreground" : "text-foreground/70")}>
                    {rule.label}
                  </span>
                  {isDisabled ? (
                    <button
  ...
  return (
    <div className="px-6 py-3 border-b border-border/30 last:border-0">
      <div className="flex items-center gap-1.5 mb-2">
        ...
        <span className="text-xs text-muted-foreground/60 font-data">({count})</span>
        {hint && <span className="text-xs text-muted-foreground/50 ml-1">· {hint}</span>}
      </div>
...
function RuleRow({
  ...
  return (
    <div className="flex items-center justify-between py-1.5 px-3 rounded-lg bg-card border border-border">
      <div className="flex items-center gap-2.5 min-w-0">
        <StageBadge stage={stage} />
        <span className="text-sm text-foreground truncate">{label}</span>
        {isGlobal && (
          <span
            className="text-xs font-semibold text-primary bg-accent px-1.5 py-0.5 rounded shrink-0"
        ...
        {matchCount !== null && (
          <span className="text-xs text-muted-foreground font-data shrink-0">{matchCount} rows</span>
        )}
      </div>
      <div className="flex items-center gap-0.5 shrink-0 ml-3">
        {onPromote && !isGlobal && (
          <button
            onClick={onPromote}
            disabled={promoting}
            className="p-1 hover:text-primary text-muted-foreground/50 transition-colors"
